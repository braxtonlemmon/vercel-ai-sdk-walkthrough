...
async function compareOutputs() {
  ...
  const { text } = await generateText({
    ...
    prompt: `Extract all the names from this text: ${namesText}`,
  });
  ...
  const appointmentSchema = z.object({
    ...
    time: z.string().nullable().describe("The time of the meeting"),
    location: z.string().nullable().describe("The location of the meeting"),
